...
class ToolRegistry:
    ...
    def execute_guarded(self, name, authorization=None, fingerprint=None,
        ...
        if name in {"computer_observe", "computer_input", "computer_focus"}:
...
def register_all_tools():
    from core.computer_control import computer_observe, computer_input, computer_focus
    ...
    registry.register("computer_focus", "Bring only the locally selected task window forward. Observe again before input. Windows may decline focus.", False, computer_focus, risk="high")
    ...
    try:
        ...
        registry.register("record_sandbox_validation", "Evaluate evidence and bind it to one sandbox's exact clean commit. sandbox_id= checks=[...] threshold=95", False, record_sandbox_validation, risk="low")
        registry.register("promote_validation_sandbox", "Fast-forward a clean unchanged source to one exact validated sandbox commit. sandbox_id= confirm=yes", False, promote_validation_sandbox, risk="high")
